...
router.post('/generateReport', auth.authenticateToken, (req, res) => {
    ...
    let createdBy = res.locals.email || "admin";
    let query = "insert into bill (name, uuid, email, contactNumber, paymentMethod, total, productDetails, createdBy) values(?,?,?,?,?,?,?,?)";
    connection.query(query, [orderDetails.name, generatedUuid, orderDetails.email, orderDetails.contactNumber, orderDetails.paymentMethod, orderDetails.totalAmount, orderDetails.productDetails, createdBy], (err, results) => {
        if(!err) {
...
router.delete('/delete/:id', auth.authenticateToken, (req, res, next) => {
    ...
    connection.query(query, [id], (err, results) => {
        if(!err) {
            ...
            return res.status(200).json({message: "Bill is deleted successfully"});
        }
        else {
            return res.status(500).json(err);
        }
